...
export function buildGeometry(_line, _tile, _id) {
    ...
    const distance = (GLOBE.webglUnitsByMeter * 0.00001) * (_line.props.width * 2);
...
function inflate(coords, distance) {
    ...
    let polygon = shell.buffer(distance, Jsts.operation.buffer.BufferParameters.CAP_FLAT);
    ...
    if (isClosed) {
        let polygon = shell.buffer(distance * -1, Jsts.operation.buffer.BufferParameters.CAP_FLAT);
